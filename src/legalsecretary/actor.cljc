(ns legalsecretary.actor
  "LegalSecretaryActor — the ISCO-08 3342 community legal secretarial
  actor as a `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one legal-secretarial operation
  ...
  The unconditional invariant: the LegalSecretaryAdvisor can never
  directly commit a record the LegalSecretaryGovernor refuses —
  every commit-record! call is gated behind `:decide`. In particular,
  the advisor can never finalize disclosure of privileged/confidential
  case information, provide legal advice, or set a filing
  deadline/legal strategy without attorney sign-off, because no such
  op exists in the governor's closed allowlist."
...
(defn build-graph
  "Build a compiled LegalSecretaryActor graph. `store` implements
  `legalsecretary.store/Store`. `advisor` implements
  `legalsecretary.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:attorney-id (:attorney-id request)
                                    :op (:op proposal)
